...
// Initialize an object to track resources sent by each village
var resourcesSentByVillage = {};
function sendResource(sourceID, targetID, woodAmount, stoneAmount, ironAmount, rowNr) {
    ...
    // Record the resources sent by this village
    if (!resourcesSentByVillage[sourceID]) {
        resourcesSentByVillage[sourceID] = { wood: 0, stone: 0, iron: 0 };
    }
    resourcesSentByVillage[sourceID].wood += woodAmount;
    resourcesSentByVillage[sourceID].stone += stoneAmount;
    resourcesSentByVillage[sourceID].iron += ironAmount;

    // Display updated sent resources for this village
    updateVillageSentResourcesDisplay(sourceID);
...
// Function to display sent resources for a village
function updateVillageSentResourcesDisplay(villageID) {
    if (!resourcesSentByVillage[villageID]) return;

    // Get the display element for this village
    var displayElement = $("#resourcesSent_" + villageID);
    if (!displayElement.length) {
        // If the display element does not exist, create it
        $("#appendHere").append(`
            <tr id="resourcesSent_${villageID}">
                <td colspan="2">${villagesData.find(v => v.id == villageID).name}</td>
                <td colspan="5">
                    <span class="icon header wood"> ${numberWithCommas(resourcesSentByVillage[villageID].wood)} </span>
                    <span class="icon header stone"> ${numberWithCommas(resourcesSentByVillage[villageID].stone)} </span>
                    <span class="icon header iron"> ${numberWithCommas(resourcesSentByVillage[villageID].iron)} </span>
                </td>
            </tr>
        `);
    } else {
        // Update the display element with the new values
        displayElement.html(`
            <td colspan="2">${villagesData.find(v => v.id == villageID).name}</td>
            <td colspan="5">
                <span class="icon header wood"> ${numberWithCommas(resourcesSentByVillage[villageID].wood)} </span>
